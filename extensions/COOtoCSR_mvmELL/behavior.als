...
/*
 * Specification of the COO, ELL and CSR abstraction functions, dense and 
...
-- COO abstraction fucntion
pred alpha [d: COO, m: Matrix] {
  m.rows = d.rows
  m.cols = d.cols
  m.vals = {
    i: range[d.rows], j: range[d.cols], v: Value |
      let k = d.col_ptr[j]  |
        one k => v = d.A[k] else v = Zero
  }
}
